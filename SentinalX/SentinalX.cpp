...
void checkAnomalies(const std::vector<ProcessInfo>& processes, bool monitorOpen) {
    ...
    for (const auto& p : processes) {
        ...
        ThreatScore threat;
        if (avgCPU > 40.0) threat.add("Sustained high CPU (" + std::to_string((int)avgCPU) + "% avg)");
        if (p.memoryMB > 500) threat.add("High memory usage (" + std::to_string(p.memoryMB) + " MB)");
        HANDLE h = OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, FALSE, p.pid);
        if (h) {
            int conns = countNetworkConnections(p.pid);
            if (conns > 5) threat.add("High network activity (" + std::to_string(conns) + " connections)");
            if (isHighDiskIO(p.pid, h)) threat.add("Abnormal disk I/O (>10MB/interval)");
            if (isHeadless(p.pid)) threat.add("No visible window - headless");
            CloseHandle(h);
        }
        if (!threat.hasResourceSignal() || threat.score == 1) { silent++; continue; }
        if (threat.score >= 2) {
            std::string dets = "PID: " + std::to_string(p.pid) + "\n";
...
void scannerThread() {
    ...
    addLog("[*] Signature scan started in background...");
    ...
    while (scanning) {
        ...
        if (currentTick % 4 == 0) {
            char buf[80];
            sprintf_s(buf, "[%llu] Tracking %d | Watching: %d",
                currentTick, (int)totalProcesses, (int)watchingCount);
...
void drawArc(sf::RenderWindow& w, sf::Vector2f c, float r,
    float s, float e, sf::Color col) {
...
int main() {
    ...
    float        arcAngle = -90.f;
    float        glowPulse = 0.f;
    float        alertFlipT = 0.f;
    float        burstTimer = 0.f;
    int          alertFlipN = 0;
    bool         alertFlipRed = true;
    ...
    while (window.isOpen()) {
        ...
        // new threat — bring window to front
        ...
        // after alert done remove always-on-top
        if (state == SCANNING) {
            SetWindowPos(window.getSystemHandle(), HWND_NOTOPMOST, 0, 0, 0, 0,
                SWP_NOMOVE | SWP_NOSIZE);
        }
        ...
        if (state == ALERT) {
            ...
            for (auto& p : particles) {
                if (particleMode == BURST_MODE) {
                    ...
                    partShape.setFillColor(sf::Color(220, 40, 40, (sf::Uint8)std::min(255.f, a)));
                    ...
                    partShape.setOrigin(p.radius, p.radius);
                    partShape.setPosition(p.pos);
                    sf::Color pc = (state == SCANNING || state == TRANSITIONING)
